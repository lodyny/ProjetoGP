    ...
    <div class="text-center" style="margin-top:20px;">
        ...
        <v-layout align-center justify-center row>
            <v-flex xs3>
                ...
                <v-form method="post">
                    <v-text-field class="ma-0" label="Email"
                    v-model="emailC"
                    ...
                    <v-text-field class="ma-0" label="Password"
                    ...
                    <v-text-field class="ma-0" label="Confirmar Password"
                    :append-icon="e1 ? 'visibility' : 'visibility_off'"
                          @click:append="() => (e1 = !e1)"
                          :type="e1 ? 'password' : 'text'"
                    ...
                    <v-tooltip slot="append-outer" right color="white" :activator="!validepwC ? '' : 'a'">
                      ...
                      <span><font color="black">As passwords tem de ser iguais</font></span>
                    </v-tooltip>
                    </v-text-field>
                    ...
                    <v-tooltip slot="append-outer" right color="white" :activator="existsuser ? (valideuser ? 'a' : '') : ''">
                      ...
                      <span><font color="black">Minimo 6 caracteres</font></span>
                    </v-tooltip>
                    </v-text-field>
                    ...
                    <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition" offset-y full-width min-width="290px">
                        <template v-slot:activator="{ on }">
                            <v-text-field class="ma-0 text-colort" v-model="date" label="Data de nascimento"  readonly v-on="on">
                            <v-icon slot="append" color="blue">event</v-icon></v-text-field>
                        </template>
                        <v-date-picker ref="picker" v-model="date" no-title :max="deltaDate(new Date(), 0, 0, -16).toISOString().substr(0, 10)" min="1950-01-01" @change="save"></v-date-picker>
                    </v-menu>
                    ...
                    <v-tooltip slot="append-outer" right color="white" :activator="!validephone ? '' : 'a'">
                      ...
                      <span><font color="black">Número inválido</font></span>
                    </v-tooltip>
                    </v-text-field>
                <v-btn
                  block 
...
export default {
        ...
        return {
          ...
          errors_pw: null,
            ...
            rules: {
              ...
              pwValidate: value => {
                const booleanResult = !/^[a-zA-Z0-9]+$/.test(value) && /\d/.test(value) && /[a-z]/.test(value) && /[A-Z]/.test(value) && value.length > 5;
    ...
    computed: {
      getAllConfirmations() {
        let allConfirmations = [this.validemail, this.validepw, this.validepwC, this.valideuser, this.validephone, !!this.date]
    ...
    methods: {
      ...
      pwMatchError () { 
        return  /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(this.emailC);
      },
      ...
      pwmessagebuilder(pw){
        ...
        this.pwmessage += '<font color="black"> Pelo menos um caracter alfanumérico.</font> <br/>';
        ...
        this.pwmessage += '<font color="black"> Pelo menos um número (0-9).</font> <br/>';
        ...
        this.pwmessage += '<font color="black"> Pelo menos uma letra minuscula (a-z).</font> <br/>';
        ...
        this.pwmessage += '<font color="black"> Pelo menos uma letra maiuscula (A-Z).</font> <br/>';
